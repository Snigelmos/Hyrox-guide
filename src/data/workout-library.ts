/**
 * Workout library dataset.
 *
 * Used by /workouts/ to render the 23-session training library inline
 * below the named benchmarks. Originally lived at /training/workouts/
 * before the SEO consolidation pass moved it to a single canonical
 * /workouts/ hub.
 */

export type WorkoutCategoryId =
  | "full-simulations"
  | "running"
  | "station-work"
  | "strength-power";

export type WorkoutType = "hyrox-specific" | "general";

export interface WorkoutCategory {
  id: WorkoutCategoryId;
  label: string;
  icon: string;
  color: string;
}

export interface LibraryWorkout {
  id: string;
  category: WorkoutCategoryId;
  categoryLabel: string;
  workoutType: WorkoutType;
  name: string;
  time: string;
  level: string;
  equipment: string;
  tagColor: string;
  description: string;
  warmup?: string;
  bullets?: string[];
  table?: { headers: string[]; rows: string[][] };
  pacing?: { headers: string[]; rows: string[][] };
  benchmarks?: { label: string; time: string; color: string }[];
  notes?: string[];
}

export const LIBRARY_CATEGORIES: WorkoutCategory[] = [
  { id: "full-simulations", label: "Full Simulations", icon: "🏁", color: "text-sky-400 bg-sky-400/10 border-sky-400/20" },
  { id: "running",          label: "Running",          icon: "🏃", color: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20" },
  { id: "station-work",     label: "Station Work",     icon: "🏋️", color: "text-amber-400 bg-amber-400/10 border-amber-400/20" },
  { id: "strength-power",   label: "Strength & Power", icon: "💥", color: "text-red-400 bg-red-400/10 border-red-400/20" },
];

export const WORKOUT_TYPE_CONFIG: Record<WorkoutType, { label: string; shortLabel: string; color: string }> = {
  "hyrox-specific": {
    label: "Hyrox specific",
    shortLabel: "Hyrox",
    color: "text-accent bg-accent/10 border-accent/20",
  },
  general: {
    label: "General",
    shortLabel: "General",
    color: "text-violet-300 bg-violet-400/10 border-violet-400/20",
  },
};

export const LIBRARY_WORKOUTS: LibraryWorkout[] = [
  {
    id: "full-send",
    category: "full-simulations",
    categoryLabel: "Full Simulation",
    workoutType: "hyrox-specific",
    name: '"The Full Send" — Complete Race Simulation',
    time: "60–75 min",
    level: "Intermediate–Advanced",
    equipment: "Full gym",
    tagColor: "text-sky-400 bg-sky-400/10 border-sky-400/20",
    description: "The closest you can get to race day without an entry fee. Complete every station in race order with 1 km treadmill runs between each. Do this once every 2–3 weeks — it's the single most effective Hyrox training session you can do.",
    warmup: "800 m easy jog, 10 leg swings each side, 10 arm circles, 5 inchworms",
    table: {
      headers: ["Order", "Exercise", "Target"],
      rows: [
        ["1","Treadmill Run","1 km"],["2","SkiErg","1,000 m"],["3","Treadmill Run","1 km"],["4","Sled Push","50 m at race weight"],
        ["5","Treadmill Run","1 km"],["6","Sled Pull","50 m at race weight"],["7","Treadmill Run","1 km"],["8","Burpee Broad Jumps","80 m (or 30 reps)"],
        ["9","Treadmill Run","1 km"],["10","Rowing","1,000 m"],["11","Treadmill Run","1 km"],["12","Farmers Carry","200 m at race weight"],
        ["13","Treadmill Run","1 km"],["14","Sandbag Lunges","100 m"],["15","Treadmill Run","1 km"],["16","Wall Balls","100 reps men / 75 reps women"],
      ],
    },
    pacing: {
      headers: ["Station","Competitive","Average","Beginner"],
      rows: [
        ["Each 1 km run","4:30","5:15","6:00"],
        ["SkiErg","4:00","4:40","5:45"],["Sled Push","2:00","3:00","4:15"],["Sled Pull","2:00","3:00","4:15"],
        ["Burpee Broad Jumps","4:00","5:15","7:30"],["Rowing","3:45","4:20","5:20"],
        ["Farmers Carry","2:30","3:30","5:00"],["Sandbag Lunges","4:00","5:15","7:00"],["Wall Balls","4:00","5:15","7:00"],
      ],
    },
    notes: ["Women's Open: Add ~15–30 s per station and ~30–45 s per 1 km run.", "Beginners: Cut runs to 800 m, halve burpee distance, reduce wall balls to 50, use lighter weights."],
  },
  {
    id: "front-four",
    category: "full-simulations",
    categoryLabel: "Half Simulation",
    workoutType: "hyrox-specific",
    name: '"The Front Four" — First Half Simulation',
    time: "30–40 min",
    level: "All levels",
    equipment: "SkiErg, sled, treadmill",
    tagColor: "text-sky-400 bg-sky-400/10 border-sky-400/20",
    description: "Focus on the first four stations where most people go out too fast and blow up. Practise starting conservative — if you sprint the SkiErg, the sled push will destroy you.",
    table: {
      headers: ["Order","Exercise","Target"],
      rows: [
        ["1","Treadmill Run","1 km"],["2","SkiErg","1,000 m"],["3","Treadmill Run","1 km"],
        ["4","Sled Push","50 m"],["5","Treadmill Run","1 km"],["6","Sled Pull","50 m"],
        ["7","Treadmill Run","1 km"],["8","Burpee Broad Jumps","80 m or 30 reps"],
      ],
    },
    notes: ["Key focus: Aim for 75% effort on the first two stations.", "Scale: Reduce runs to 600 m, use light sled weight, do 20 burpee broad jumps."],
  },
  {
    id: "back-four",
    category: "full-simulations",
    categoryLabel: "Half Simulation",
    workoutType: "hyrox-specific",
    name: '"The Back Four" — Second Half Simulation',
    time: "30–40 min",
    level: "All levels",
    equipment: "Rower, kettlebells, sandbag, wall ball, treadmill",
    tagColor: "text-sky-400 bg-sky-400/10 border-sky-400/20",
    description: "The back half is where races are won or lost. Your grip is gone, legs are heavy, and you still have four brutal stations left. Practise switching the sandbag shoulder every 25 m.",
    table: {
      headers: ["Order","Exercise","Target"],
      rows: [
        ["1","Treadmill Run","1 km"],["2","Rowing","1,000 m"],["3","Treadmill Run","1 km"],
        ["4","Farmers Carry","200 m"],["5","Treadmill Run","1 km"],["6","Sandbag Lunges","100 m"],
        ["7","Treadmill Run","1 km"],["8","Wall Balls","100 reps men / 75 reps women"],
      ],
    },
    notes: ["Scale: Reduce runs to 600 m, carry lighter kettlebells, do 50 wall balls, lunge 50 m."],
  },

  {
    id: "cardio-killer",
    category: "running",
    categoryLabel: "Cardio",
    workoutType: "general",
    name: '"The Cardio Killer" — Maximum Heart Rate',
    time: "30 min",
    level: "All levels",
    equipment: "SkiErg, rower, treadmill",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "Pure cardiovascular punishment. This gets your heart rate to race-day levels and keeps it there. Two parts: an EMOM (every minute on the minute) followed by intervals.",
    warmup: "400 m jog, 10 burpees at easy pace, dynamic stretching",
    bullets: [
      "Part 1 — EMOM for 12 minutes (repeat 4 times): Minute 1: 15 cal SkiErg · Minute 2: 15 cal Row · Minute 3: 6 Burpee Broad Jumps",
      "Part 2 — 4 Intervals: 400 m treadmill at 90% · 250 m Row at max · 60 s rest",
    ],
    notes: ["Scale: Reduce EMOM calories to 10, burpees to 4. In intervals, run 200 m and row 150 m."],
  },
  {
    id: "race-pace-intervals",
    category: "running",
    categoryLabel: "Running",
    workoutType: "hyrox-specific",
    name: '"Race Pace Intervals" — 8×1 km at Target Pace',
    time: "45–55 min",
    level: "Intermediate–Advanced",
    equipment: "Treadmill or track",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "Hyrox running totals 8 km across 8 separate 1 km efforts with functional work in between. This session trains your body to hit target 1 km pace over and over, even when fatigued. The 90-second rest simulates the time you spend transitioning to a station.",
    warmup: "10 min easy jog, 4 strides at race pace",
    table: {
      headers: ["Rep","Distance","Pace","Rest"],
      rows: [
        ["1–3","1 km","5K race pace","90 s"],
        ["4–6","1 km","5K race pace + 10 s/km","90 s"],
        ["7–8","1 km","Max sustainable effort","90 s"],
      ],
    },
    pacing: {
      headers: ["Goal Hyrox Time","Target 1 km Pace"],
      rows: [
        ["Under 75 min","4:15–4:30/km"],
        ["75–90 min","4:45–5:00/km"],
        ["90–110 min","5:15–5:45/km"],
        ["First finisher","5:45–6:30/km"],
      ],
    },
    notes: ["Do this once a week at most — it's high intensity.", "Replace intervals 7–8 with easy 1 km if you're a beginner."],
  },
  {
    id: "run-bricks",
    category: "running",
    categoryLabel: "Running",
    workoutType: "hyrox-specific",
    name: '"Hyrox Run Bricks" — Run Into Stations',
    time: "35–45 min",
    level: "All levels",
    equipment: "Treadmill, wall ball, rower",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "The specific challenge of Hyrox is going straight from running into a functional station without slowing down. This session trains that transition — your legs are already burning when you hit the station. Repeat 4 rounds with minimal rest.",
    table: {
      headers: ["Exercise","Target"],
      rows: [
        ["Treadmill Run","1 km at race pace"],
        ["Wall Balls","25 reps"],
        ["Treadmill Run","1 km at race pace"],
        ["Rowing","500 m"],
        ["Treadmill Run","1 km at race pace"],
        ["Burpee Broad Jumps","15 reps"],
        ["Treadmill Run","1 km at race pace"],
        ["Farmers Carry","100 m"],
      ],
    },
    notes: ["Rest 2–3 minutes between rounds. Do 2 rounds for beginners.", "Focus on the first 30 seconds of each station — don't let the run slow your station start."],
  },
  {
    id: "norwegian-4x4",
    category: "running",
    categoryLabel: "General Running",
    workoutType: "general",
    name: '"Norwegian 4×4" — VO2 Max Intervals',
    time: "35–45 min",
    level: "All levels",
    equipment: "Treadmill, track, bike, or rower",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "A classic aerobic power session: four hard 4-minute intervals with enough recovery to keep the quality high. Use it to raise your ceiling without adding more race-specific fatigue.",
    warmup: "10 min easy, then 3 × 20 s strides or pickups",
    table: {
      headers: ["Block","Target","Recovery"],
      rows: [
        ["Warm-up","10 min easy + 3 short pickups","Walk/jog 60 s"],
        ["Intervals","4 × 4 min at 90–95% HR max","3 min easy jog"],
        ["Cool-down","8–10 min easy","Keep it conversational"],
      ],
    },
    notes: ["Hard should feel controlled, not like an all-out sprint.", "If you are new to intervals, start with 3 rounds and build to 4."],
  },
  {
    id: "zone-2-builder",
    category: "running",
    categoryLabel: "General Running",
    workoutType: "general",
    name: '"Zone 2 Builder" — Aerobic Base Session',
    time: "45–60 min",
    level: "All levels",
    equipment: "Road, treadmill, bike, rower, or SkiErg",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "The simple session most athletes skip. Build the aerobic base that lets you recover between stations and handle more total weekly training without burning out.",
    bullets: [
      "45–60 min continuous at conversational pace.",
      "Keep effort at roughly 60–70% HR max or a 3–4/10 RPE.",
      "Finish feeling like you could keep going for another 15 minutes.",
    ],
    notes: ["Use bike, rower, or SkiErg if your legs need a break from running.", "Do not turn this into a tempo run — the value is in staying easy."],
  },
  {
    id: "tempo-threshold-sandwich",
    category: "running",
    categoryLabel: "General Running",
    workoutType: "general",
    name: '"Tempo Threshold Sandwich" — Comfortably Hard Run',
    time: "40–50 min",
    level: "Intermediate",
    equipment: "Treadmill, track, or road",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "A controlled threshold workout for holding speed without redlining. It improves the pace you can sustain between stations and pairs well with a lighter station day.",
    table: {
      headers: ["Segment","Duration","Effort"],
      rows: [
        ["Easy run","10–15 min","Conversational"],
        ["Tempo","20 min","7/10 effort, controlled breathing"],
        ["Easy run","10–15 min","Relaxed cool-down"],
      ],
    },
    notes: ["You should finish knowing you had one more 5-minute block available.", "Beginners can split the tempo into 2 × 10 min with 2 min easy jog."],
  },
  {
    id: "compromised-kilometers",
    category: "running",
    categoryLabel: "Compromised Running",
    workoutType: "hyrox-specific",
    name: '"Compromised Kilometers" — Run After Work',
    time: "45–60 min",
    level: "Intermediate–Advanced",
    equipment: "Treadmill, rower, wall ball, kettlebells",
    tagColor: "text-emerald-400 bg-emerald-400/10 border-emerald-400/20",
    description: "Hyrox is not just running; it is running while your legs, grip, and breathing are already compromised. This session teaches you to find race pace again after station fatigue.",
    table: {
      headers: ["Round","Station Primer","Run"],
      rows: [
        ["1","500 m Row","1 km at race pace"],
        ["2","25 Wall Balls","1 km at race pace"],
        ["3","100 m Farmers Carry","1 km at race pace"],
        ["4","15 Burpee Broad Jumps","1 km at race pace"],
        ["Optional 5–6","Repeat weakest primer","1 km controlled"],
      ],
    },
    notes: ["Rest 90 s after each run.", "Keep the station primer smooth; the goal is a quality run under fatigue."],
  },

  {
    id: "push-grind",
    category: "station-work",
    categoryLabel: "Station Focus",
    workoutType: "hyrox-specific",
    name: '"Push & Grind" — Legs and Shoulders',
    time: "30 min",
    level: "All levels",
    equipment: "Sled, wall ball, SkiErg",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "A 4-round circuit that hammers the push stations. Your legs and shoulders will be on fire. Total volume: 150 m sled push, 70 wall balls, 1,400 m SkiErg.",
    warmup: "400 m jog, 10 bodyweight squats, 10 push-ups, 20 shoulder circles",
    table: {
      headers: ["Exercise","Round 1","Round 2","Round 3","Round 4"],
      rows: [
        ["Sled Push","50 m","50 m","25 m","25 m"],
        ["Wall Balls","25 reps","20 reps","15 reps","10 reps"],
        ["SkiErg","500 m","400 m","300 m","200 m"],
      ],
    },
    notes: ["4 rounds, 90 seconds rest between rounds.", "Scale: Use 60–70% of race weight on sled. Beginners rest 2 minutes."],
  },
  {
    id: "grip-carry",
    category: "station-work",
    categoryLabel: "Station Focus",
    workoutType: "hyrox-specific",
    name: '"Grip & Carry" — Forearm Endurance',
    time: "30 min",
    level: "All levels",
    equipment: "Sled with rope, kettlebells, sandbag, pull-up bar",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "Most people fail Hyrox because their grip gives out by station 6. This session trains the forearm endurance your sled pull, farmers carry, and lunges demand.",
    warmup: "400 m jog, wrist circles, 30 s dead hang, 10 light KB swings",
    table: {
      headers: ["Exercise","Reps / Distance"],
      rows: [
        ["Sled Pull","50 m at race weight"],
        ["Farmers Carry","100 m at race weight"],
        ["Sandbag Lunges","50 m"],
        ["Dead Hang","Max time (aim 30–60 s)"],
      ],
    },
    notes: ["4 rounds, 2 minutes rest between rounds.", "Finisher: 3 sets of towel pull-ups (max reps) or plate pinch holds.", "Scale: Reduce sled to 25 m, carry to 50 m, lunges to 25 m."],
  },
  {
    id: "burpee-basher",
    category: "station-work",
    categoryLabel: "Station Focus",
    workoutType: "hyrox-specific",
    name: '"Burpee Basher" — Survive Station 4',
    time: "25 min",
    level: "All levels",
    equipment: "Floor space, treadmill",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "Burpee Broad Jumps destroy most Hyrox athletes because they go too hard early and can't sustain 80 m. This session builds the pacing discipline and specific endurance to get through all 80 m without stopping.",
    warmup: "5 min easy jog, 10 inchworms",
    bullets: [
      "Set 1: 20 Burpee Broad Jumps at easy/conversational pace — learn the rhythm.",
      "Set 2: 1 km treadmill run (race pace) → immediately into 20 Burpee Broad Jumps.",
      "Set 3: 1 km treadmill run (race pace) → 30 Burpee Broad Jumps → 1 km cool-down run.",
      "Finisher: 3 × 10 Burpees (standard, no jump) for upper-body fatigue.",
    ],
    notes: ["Rest 2 min between sets.", "Aim for constant forward motion — never stop mid-rep. Slow is fine.", "Scale: Do standard burpees with a small hop forward instead of broad jump."],
  },
  {
    id: "lunch-break",
    category: "station-work",
    categoryLabel: "Quick Combo",
    workoutType: "hyrox-specific",
    name: '"Lunch Break Hyrox" — 20-Minute Blitz',
    time: "20 min",
    level: "All levels",
    equipment: "SkiErg, rower, wall ball, floor space",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "When you only have 20 minutes but want to feel like you just raced. 4 rounds for time — no warm-up needed beyond the first easy round.",
    table: {
      headers: ["Exercise","Reps / Distance"],
      rows: [["SkiErg","250 m"],["Rowing","250 m"],["Wall Balls","10 reps"],["Burpee Broad Jumps","10 reps"],["Treadmill Run","400 m"]],
    },
    benchmarks: [
      { label: "Elite", time: "under 16 min", color: "text-emerald-400" },
      { label: "Competitive", time: "16–20 min", color: "text-sky-400" },
      { label: "Average", time: "20–25 min", color: "text-amber-400" },
      { label: "Beginner", time: "25–30 min", color: "text-red-400" },
    ],
    notes: ["No SkiErg? Substitute 30 cal assault bike. No sled? Heavy dumbbell bear crawls (10 m)."],
  },
  {
    id: "sled-capacity-builder",
    category: "station-work",
    categoryLabel: "Station Focus",
    workoutType: "hyrox-specific",
    name: '"Sled Capacity Builder" — Push/Pull Repeats',
    time: "35–45 min",
    level: "Intermediate",
    equipment: "Sled, turf, rope",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "Sled work gets ugly when the load is just heavy enough to break your rhythm. This session builds repeatable sled efforts without turning every rep into a max-effort grind.",
    warmup: "5 min easy jog, hip openers, 3 light sled push build-up reps",
    table: {
      headers: ["Set","Push","Pull","Rest"],
      rows: [
        ["1–2","25 m at 70% race load","25 m at 70% race load","90 s"],
        ["3–5","25 m at race load","25 m at race load","2 min"],
        ["6","50 m smooth push","50 m smooth pull","Full recovery"],
      ],
    },
    notes: ["Keep steps short and constant on the push.", "If form breaks, reduce load before adding rest."],
  },
  {
    id: "wall-ball-ladder",
    category: "station-work",
    categoryLabel: "Station Focus",
    workoutType: "hyrox-specific",
    name: '"Wall Ball Ladder" — Finish Strong',
    time: "25–35 min",
    level: "All levels",
    equipment: "Wall ball, treadmill or rower",
    tagColor: "text-amber-400 bg-amber-400/10 border-amber-400/20",
    description: "A focused wall ball session for the station that ends your race. The ladder format builds confidence in larger sets while forcing you to control breathing before picking the ball back up.",
    table: {
      headers: ["Round","Buy-in","Wall Balls"],
      rows: [
        ["1","400 m run or 500 m row","15 reps"],
        ["2","400 m run or 500 m row","20 reps"],
        ["3","400 m run or 500 m row","25 reps"],
        ["4","400 m run or 500 m row","30 reps"],
        ["5","Easy walk 2 min","Max unbroken set"],
      ],
    },
    notes: ["Rest only as needed between rounds.", "Beginners can use 10–15–20–20 and skip the max set."],
  },

  {
    id: "posterior-chain",
    category: "strength-power",
    categoryLabel: "Strength",
    workoutType: "general",
    name: '"Posterior Chain Power" — Sled & Pull Strength',
    time: "45–55 min",
    level: "All levels",
    equipment: "Barbell, kettlebells, rower",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "Your sled push, sled pull, and farmers carry all run on posterior chain strength. This heavy gym day builds the deadlift, hip-hinge, and row power that directly transfers to your worst stations. Do this once a week in your base training phase.",
    warmup: "400 m easy row, 10 KB Romanian deadlifts, 10 glute bridges",
    table: {
      headers: ["Exercise","Sets × Reps","Rest"],
      rows: [
        ["Deadlift (or trap bar DL)","4 × 4–6 @ 80–85% 1RM","3 min"],
        ["Romanian Deadlift","3 × 8–10 @ moderate","90 s"],
        ["Single-leg KB Deadlift","3 × 10 each side","60 s"],
        ["Seated Cable Row (heavy)","4 × 8","90 s"],
        ["Farmers Carry","4 × 40 m heavy","90 s"],
        ["GHD Back Extensions / Good Mornings","3 × 12","60 s"],
      ],
    },
    notes: ["Heavy = 80%+ of your 1 rep max.", "Keep rep quality high — no rounding the lower back on deadlifts.", "Substitute trap bar if you're newer to conventional deadlifts."],
  },
  {
    id: "push-pull-power",
    category: "strength-power",
    categoryLabel: "Strength",
    workoutType: "general",
    name: '"Upper Push-Pull" — SkiErg & Wall Ball Strength',
    time: "45–55 min",
    level: "All levels",
    equipment: "Barbell, dumbbells, cable machine, pull-up bar",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "SkiErg is all about lat strength and shoulder endurance. Wall balls need shoulder stability under load. This session builds the vertical pulling and pressing capacity your upper body stations demand.",
    warmup: "400 m easy row, 10 shoulder circles each direction, band pull-aparts",
    table: {
      headers: ["Exercise","Sets × Reps","Rest"],
      rows: [
        ["Weighted Pull-ups (or lat pulldown)","4 × 5–6","3 min"],
        ["Incline DB Press or Bench Press","4 × 6–8","2 min"],
        ["Single-arm DB Row","3 × 10 each side","60 s"],
        ["Cable Tricep Pressdown","3 × 12","60 s"],
        ["Face Pulls (cable)","3 × 15","60 s"],
        ["Overhead Carry (plate or KB)","3 × 30 m","60 s"],
      ],
    },
    notes: ["Overhead Carry directly mimics the shoulder demand of wall balls.", "Pull-ups are the best single exercise for SkiErg. If you can't do them yet, lat pulldowns are the right sub."],
  },
  {
    id: "squat-lunge-power",
    category: "strength-power",
    categoryLabel: "Strength",
    workoutType: "general",
    name: '"Squat & Lunge Power" — Wall Balls & Sandbag Lunges',
    time: "45–50 min",
    level: "All levels",
    equipment: "Barbell, dumbbells, sandbag, wall ball",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "Wall Balls (75–100 reps) and Sandbag Lunges (100 m) are stations 7 and 8 — when your legs have nothing left. Strength-train your quad and glute capacity to still push hard at the end of a race.",
    warmup: "400 m easy jog, 10 goblet squats, 10 walking lunges, hip flexor stretch",
    table: {
      headers: ["Exercise","Sets × Reps","Rest"],
      rows: [
        ["Back Squat","4 × 5 @ 80% 1RM","3 min"],
        ["Bulgarian Split Squat (DB)","3 × 8 each leg","90 s"],
        ["Walking Lunges (weighted vest or DBs)","3 × 20 steps","90 s"],
        ["Goblet Squat to Pause (3 s bottom)","3 × 12","60 s"],
        ["Wall Ball — light (6–8 kg)","3 × 25 reps","90 s"],
        ["Sandbag Carry + Lunge finisher","2 × 40 m lunge","2 min"],
      ],
    },
    notes: ["The pause goblet squat builds the bottom position strength for wall balls.", "Do sandbag carry + lunge last when you're already fatigued — that's race specificity.", "Scale: Remove load, focus on technique and range of motion."],
  },
  {
    id: "engine-emom-30",
    category: "strength-power",
    categoryLabel: "Conditioning",
    workoutType: "general",
    name: '"Engine EMOM 30" — CrossFit-Inspired Conditioning',
    time: "30 min",
    level: "All levels",
    equipment: "Rower, wall ball, kettlebell, floor space",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "A sustainable mixed-modal EMOM that builds work capacity without needing race-specific equipment. The goal is to finish each minute with 10–20 seconds to breathe.",
    table: {
      headers: ["Minute","Work"],
      rows: [
        ["1","12–16 cal Row"],
        ["2","12 Wall Balls"],
        ["3","12 KB Swings"],
        ["4","8 Burpees"],
        ["5","Rest or easy nasal breathing"],
      ],
    },
    notes: ["Repeat 6 times for 30 minutes.", "Scale reps so you never fail a minute in the first half."],
  },
  {
    id: "cindy-style-amrap",
    category: "strength-power",
    categoryLabel: "Conditioning",
    workoutType: "general",
    name: '"Cindy-Style AMRAP" — Bodyweight Capacity',
    time: "20 min",
    level: "All levels",
    equipment: "Pull-up bar or rings",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "A simple CrossFit-inspired bodyweight test for upper-body pulling, pressing, and leg stamina. It is easy to scale and useful when you cannot get full Hyrox equipment.",
    table: {
      headers: ["AMRAP 20","Movement"],
      rows: [
        ["5 reps","Pull-ups or ring rows"],
        ["10 reps","Push-ups"],
        ["15 reps","Air squats"],
      ],
    },
    benchmarks: [
      { label: "Strong", time: "20+ rounds", color: "text-emerald-400" },
      { label: "Solid", time: "15–19 rounds", color: "text-sky-400" },
      { label: "Building", time: "10–14 rounds", color: "text-amber-400" },
      { label: "Beginner", time: "under 10", color: "text-red-400" },
    ],
    notes: ["Break push-ups early before they fail.", "Use ring rows and elevated push-ups to keep movement quality high."],
  },
  {
    id: "chipper-conditioning",
    category: "strength-power",
    categoryLabel: "Conditioning",
    workoutType: "general",
    name: '"Chipper Conditioning" — Long Mixed-Modal Grind',
    time: "35–45 min",
    level: "Intermediate",
    equipment: "Rower or SkiErg, wall ball, kettlebell, dumbbells",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "A CrossFit-inspired chipper for pacing discipline. Move through one long list at a steady effort instead of sprinting the first two pieces and surviving the rest.",
    table: {
      headers: ["Order","Work"],
      rows: [
        ["1","1,000 m Row or SkiErg"],
        ["2","50 Wall Balls"],
        ["3","40 KB Swings"],
        ["4","30 Burpees"],
        ["5","20 DB Walking Lunges each leg"],
        ["6","1,000 m Row or SkiErg"],
      ],
    },
    notes: ["Cap at 45 minutes.", "Beginners can cut all reps and distances by 30–40%."],
  },
  {
    id: "db-kettlebell-grinder",
    category: "strength-power",
    categoryLabel: "Strength Conditioning",
    workoutType: "general",
    name: '"DB/KB Grinder" — Minimal-Equipment Strength Conditioning',
    time: "30–40 min",
    level: "All levels",
    equipment: "Dumbbells or kettlebells",
    tagColor: "text-red-400 bg-red-400/10 border-red-400/20",
    description: "A practical hotel-gym or busy-gym session that builds loaded carries, squatting, pressing, and trunk stiffness with only a pair of weights.",
    table: {
      headers: ["Round","Work"],
      rows: [
        ["1–5","40 m suitcase carry each side"],
        ["1–5","15 goblet squats"],
        ["1–5","10 DB or KB push press"],
        ["1–5","8 renegade rows each side"],
        ["1–5","20 walking lunges"],
      ],
    },
    notes: ["Rest 60–90 seconds between rounds.", "Choose a load you can press cleanly for all 5 rounds."],
  },
];

export const TYPED_LIBRARY_WORKOUTS = LIBRARY_WORKOUTS.map((workout) => ({
  ...workout,
  workoutTypeLabel: WORKOUT_TYPE_CONFIG[workout.workoutType].label,
  workoutTypeShortLabel: WORKOUT_TYPE_CONFIG[workout.workoutType].shortLabel,
  workoutTypeColor: WORKOUT_TYPE_CONFIG[workout.workoutType].color,
}));

export const LIBRARY_CATEGORIES_WITH_COUNTS = LIBRARY_CATEGORIES.map((category) => ({
  ...category,
  count: TYPED_LIBRARY_WORKOUTS.filter((workout) => workout.category === category.id).length,
}));

export const LIBRARY_WORKOUT_TYPE_COUNTS = Object.fromEntries(
  Object.keys(WORKOUT_TYPE_CONFIG).map((type) => [
    type,
    TYPED_LIBRARY_WORKOUTS.filter((workout) => workout.workoutType === (type as WorkoutType)).length,
  ]),
);

export const LIBRARY_TOTAL_WORKOUTS = TYPED_LIBRARY_WORKOUTS.length;
